...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filter_samples = samples.filter((sampleID) => sampleID.id == sample);
    //  5. Create a variable that holds the first sample in the array.
    first_sample = filter_samples[0];
...
// Bubble chart -- Deliverable 2
    ...
    var metatdata = data.metadata;
    var gauge_filter = metatdata.filter((sampleID) => sampleID.id == sample);
    
    // 2. Create a variable that holds the first sample in the metadata array.
    var gauge_results = gauge_filter[0];

    // 3. Create a variable that holds the washing frequency.   
    var wash_frequency = gauge_results.wfreq;
    console.log(wash_frequency);

    // 4. Create the trace for the gauge chart.
    var guageData = {
      value: parseFloat(wash_frequency),
      type: "indicator",
      mode: "gauge+number",
      gauge: {
        axis: { range: [0, 10], tickwidth: 2, tickcolor: "black", nticks: 6 },
        bar: { color: "black" },
        steps: [
          { range: [0, 2], color: "red" },
